...
class PackageValidator:
    ...
    def validate_excludes(self) -> None:
        ...
        for distro, excluded_packages in excludes.items():
            ...
            for pkg in excluded_packages:
                if not isinstance(pkg, str):
                    self.errors.append(
                        f"Excluded package on '{distro}' is not a string: {pkg}"
                    )
                    continue
    ...
    def validate_provides(self) -> None:
        ...
        for logical_name, mapping in provides.items():
            if not isinstance(mapping, dict):
                self.errors.append(
                    f"Provides '{logical_name}' is not a dict: {type(mapping)}"
                )
                continue
    ...
    def validate_required_packages(self) -> None:
        ...
        if not isinstance(required, dict):
            self.errors.append(
                f"Required packages section is not a dict: {type(required)}"
            )
            return
        ...
        for category, packages_list in required.items():
            if not isinstance(packages_list, list):
                self.errors.append(
                    f"Required.{category} is not a list: {type(packages_list)}"
                )
