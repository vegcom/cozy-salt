...
class ContainerManager:
        ...
        if project_root is None:
            ...
            if project_root is None:
                raise ContainerError(
                    "Could not find project root with docker-compose.yaml"
                )
    ...
    def _run_compose(
        ...
        if check and result.returncode != 0:
            raise ContainerError(
                f"docker-compose failed: {result.stderr or result.stdout}"
            )
    ...
    def _run_docker(
        ...
        if check and result.returncode != 0:
            raise ContainerError(
                f"docker command failed: {result.stderr or result.stdout}"
            )
    ...
    def wait_for_highstate(self, config: ContainerConfig) -> bool:
        ...
        """
        logger.info(
            f"Waiting for highstate completion (max {config.timeout_seconds}s)..."
        )
    ...
    def run_test_cycle(self, distro: str, build: bool = True) -> str:
        ...
        try:
            ...
            logger.debug(
                f"salt-call output ({len(combined)} bytes): {combined[:500]!r}"
            )
