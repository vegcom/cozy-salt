"""Debian/Ubuntu/Kali Salt Minion installer using onedir bootstrap."""
...
def install(master: str, minion_id: str, roles: list[str]) -> None:
  """Install Salt Minion on Debian/Ubuntu/Kali using onedir bootstrap.

  Uses onedir for consistency across all distros.
  """
  print("=== Installing Salt Minion on Debian/Ubuntu (onedir) ===")

  # Clean up any broken Docker repo configs (common in WSL)
  ...
  run_command(["apt-get", "install", "-y", "curl", "ca-certificates"])
...
def is_supported() -> bool:
  ...
  return any(d in content for d in ["debian", "ubuntu", "kali", "mint", "pop"])
