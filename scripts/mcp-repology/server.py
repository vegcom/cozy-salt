...
LOCAL_MAP = {
    ...
    "ripgrep": {"ubuntu": "ripgrep", "debian": "ripgrep", "rhel": "ripgrep", "arch": "ripgrep"},
    ...
    "build_essentials": {"ubuntu": "build-essential", "debian": "build-essential", "rhel": "gcc", "arch": "base-devel"},
}
...
def query_repology(project: str) -> Optional[dict]:
    ...
    try:
        ...
        for distro, repo_pattern in REPO_MAP.items():
            ...
            candidates = [c for c in candidates if c and not any(
                c.endswith(s) for s in ("-debuginfo", "-debugsource", "-doc", "-devel", "-dev")
            )]
...
def resolve_package(project: str) -> dict:
    ...
    return {
        "source": "passthrough",
        "mapping": {d: project for d in REPO_MAP.keys()}
    }
...
def main():
    ...
    tools = [
        {
            ...
            "inputSchema": {
                "type": "object",
                "properties": {
                    "project": {"type": "string", "description": "Canonical package name (e.g. 'vim', 'fd', 'ripgrep')"}
                },
                "required": ["project"]
            }
        },
    ...
    for line in sys.stdin:
        try:
            ...
            print(json.dumps({"jsonrpc": "2.0", "id": None, "error": {"code": -1, "message": str(e)}}), flush=True)
