...
find . -type f \( -name "*.sls" -o -name "*.yml" -o -name "*.yaml" \) -print0 2>/dev/null | \
  while IFS= read -r -d '' file; do
    [[ $(stat -c '%a' "$file") != 664 ]] && chmod -C 664 "$file" && ((changed++))
  done
...
find . -type f \( -name "*.sh" -o -name "*.py" \) -print0 2>/dev/null | \
  while IFS= read -r -d '' file; do
    [[ $(stat -c '%a' "$file") != 775 ]] && chmod -C 775 "$file" && ((changed++))
  done
...
echo "Paths (775)..."
find srv/salt srv/pillar provisioning scripts tests -type d -exec chmod -C 775 {} \; 2>/dev/null || true
